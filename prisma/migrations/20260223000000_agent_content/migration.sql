...
CREATE TABLE "agent_contents" (
    ...
    CONSTRAINT "agent_contents_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "users" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
-- AlterTable: add content_id to items (no FK in SQLite ALTER for existing tables; Prisma SQLite uses table recreation for FKs in some flows; here we add column only)
-- SQLite does not support adding a foreign key in ALTER TABLE. We add the column; the app will only set it to valid agent_content ids.
